...
public:
  ...
  void Enqueue(ItemType timestamp,ItemType newItem);
...
PQType<ItemType>::PQType(int max)
{
  ...
  items.timestamps= new ItemType[max];
  length = 0;
}
...
PQType<ItemType>::PQType()
{
    ...
    items.timestamps = new ItemType[maxItems ];
    length = 0;
}
...
//       from the queue; a copy is returned in item.
{
  ...
  else
  {
    ...
    ItemType timestamp = items.timestamps[0];
    items.elements[0] = items.elements[length-1];
    items.timestamps[0] = items.timestamps[length - 1];
    length--;
...
template<class ItemType>
void PQType<ItemType>::Enqueue(ItemType timestamp,ItemType newItem)
// Post: newItem is in the queue.
{
  ...
  else
  {
    ...
    items.timestamps[length - 1] = timestamp;
...
void PQType<ItemType>::print() {
    ...
    int l = length;
    while (k > 0) {
        std::cout << items.elements[k-1]<<" ";
        k--;
    }
    std::cout << std::endl;
    while (l > 0) {
        std::cout << items.timestamps[l - 1] << " ";
        l--;
    }
}